...
if np.isnan(y).any():
    ...
    # You might want to count them: log(f"NaNs in y: {np.isnan(y).sum()}")
    exit()
...
for k_val in k_range:
    ...
    mse_scores = cross_val_score(knn, X_train_scaled, y_train, cv=kfold, scoring='neg_mean_squared_error', error_score='raise')
...
if len(y_test) > 0:
    ...
    log("Warning: y_test is empty, cannot plot diagonal line for actual vs predicted.")
